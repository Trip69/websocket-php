   ...
   */
    public function connect()
    {
        ...
        if (!in_array($scheme, array('ws', 'wss')))
            throw new BadUriException("Url should have scheme ws or wss, not '$scheme' from URI '$this->socket_uri' .");
        ...
        do {
            ...
            $response .= $buffer . "\n";
            if(strlen($buffer)>20 && substr($buffer,0,20)=='Sec-WebSocket-Accept')
            {
        ...
        } while ($buffer!== false && !feof($this->socket) && $metadata['unread_bytes'] > 0);
        ...
        //set blocking to that in options (default: false)
